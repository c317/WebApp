...
<%
	ArrayList<News> listKcgl =(ArrayList<News>)request.getAttribute("listKcgl");
	ArrayList<HashMap<String, Integer>> listHotWord=(ArrayList<HashMap<String, Integer>>)request.getAttribute("listHotWord");
	ArrayList<News> listKcrd =(ArrayList<News>)request.getAttribute("listKcrd");
%>
...
<body>     
  <div id="all" class="container">
  		<%request.setAttribute("menuIndex", "Kcgl");%> 
		...
		<div class="row"> 
			<div class="col-md-3 col-xs-3 col-sm-3 col-lg-3"> 
				...
				<tr>
					...
					<font face="宋体"  color="#FFFFFF"><b>&nbsp;油气热点</b></font></th>
				</tr>
				<%
							for (int i = 0; i <7 && i<listKcrd.size(); i++) {
							String title=String.valueOf(listKcrd.get(i).getTitle());
				%>    
				<tr>	
							<td>* <a href="news?newsId=<%=listKcrd.get(i).getId()%>&moduleId=100" title="<%=title%>" target="_blank"><%if(title.length()>8)
								out.print(title.substring(0,8)+"...");
							else
								out.print(title);
							%></a>
							</td>
							<td><%=listKcrd.get(i).getPubTime() %></td>
				</tr>				
				<%
					}
				%>
  					...
  					<%
						if (listKcrd.size()<7)
						for(int iii=7-listKcrd.size();iii>0;iii--)
					{
					%>
					<tr>
						<td> &nbsp;
			...
			<div class="col-md-9 col-xs-9 col-sm-9 col-lg-9">
				...
				<tr>
					<td style="border-radius:10px 0px 0px 0px;background-color:#4fb2d1"  colspan="2">
						...
						<font face="宋体" color="#FFFFFF"><b>&nbsp;油气管理</b></font></div></td>
				...
				<%
					for (int i =0; i <listKcgl.size(); i++) {
						String title=String.valueOf(listKcgl.get(i).getTitle());
				%>

				<tr>

					<td><div align="left">
							&nbsp;> <a href="news?newsId=<%=listKcgl.get(i).getId()%>&moduleId=2" target="_blank" title="<%=title%>"><%if(title.length()>25)
								out.print(title.substring(0,25)+"...");
							else
								out.print(title);
							%></a>
						</div></td>
					<td><%=listKcgl.get(i).getOriginSource()%></td>
					<td style="text-align: center"><%=listKcgl.get(i).getPubTime()%></td>
				</tr>
				<%
					}
				%>
				<tr>
					...
					<jsp:include page="pageroll.jsp" flush="true">
						<jsp:param value="getData?moduleId=2" name="action" />
					</jsp:include></td>
				</tr>
			</table>
			</div>			  
