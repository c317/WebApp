...
<head>
<%
   int moduleId=Integer.valueOf(String.valueOf(request.getAttribute("indextitleModuleId")));
			...
			<div class="col-md-10 col-xs-10 col-sm-10 col-lg-10">
				...
				<div class="table-responsive">
					<table class="table table-bordered table-hover table-striped">
						...
						<%
							ArrayList<News> titlelist = (ArrayList<News>) request.getAttribute("title_research");
							for (int i =0; i <titlelist.size() ; i++) {
								String s=String.valueOf(titlelist.get(i).getTitle());
							if(s.length()>20){
								s=s.substring(0,20)+"...";
								}else s=s;
						%>
						...
						<tr>
							<td>
								<div align="center">
								<input type="checkbox" id="cheboxId" name="newsIds" value="<%=titlelist.get(i).getId()%>">
								</div>
							</td>
							<td>
								<div align="center"><%=titlelist.get(i).getRownumber()%></div>
							</td>
							<td>
								<div align="left">
								<a href="manageNews?newsId=<%=titlelist.get(i).getId()%>&moduleId=<%=moduleId%>" target="_blank"
										title="<%=titlelist.get(i).getTitle()%>" target="_blank"><%=s%>
								</a>
								</div>
							</td>
							<td>
								<div align="center"><%=titlelist.get(i).getPubTime()%></div>
							</td>
							<td>
								<div align="center"><%=titlelist.get(i).getOriginSource()%></div>
							</td>
							<td>
								<div align="center"><%=titlelist.get(i).getSiteSource()%></div>
							</td>
							<td>
								<div align="center">
									<img src="../images/dow.gif" width="16" height="16" />
										<a href="../manager/downloadSingle?newsId=<%=titlelist.get(i).getId()%>&moduleId=1"
											onclick>下载</a>&nbsp; &nbsp; 
									<img src="../images/edt.gif" width="16" height="16" />
										<a href="manageNewsModify?newsId=<%=titlelist.get(i).getId()%>&moduleId=5">
											修改</a>&nbsp;&nbsp;
									<img src="../images/del.gif" width="16" height="16" />
										<a href="DeleteSingle?newsId=<%=titlelist.get(i).getId()%>&moduleId=5">
											删除</a>&nbsp;&nbsp;
										<%
											String isPub="";
											String pub="";
											if(titlelist.get(i).getVisible()==0){
												isPub="已发布";
												pub="pub";
											}else if(titlelist.get(i).getVisible()==1) { 
												isPub="未发布";
												pub="nopub";
											}else { }
										%>
									...
									<img src="../images/<%=pub%>.gif" height="18" />&nbsp;<a
										href="isVisible?newsId=<%=titlelist.get(i).getId()%>&moduleId=5&visible=<%=titlelist.get(i).getVisible()%>"><%=isPub%></a>														
								</div>
							</td>
						</tr>
						<%
						...
						<tr>
							...
							<jsp:include page="searchPageroll.jsp" flush="true">
			                         <jsp:param name="action" value="manageNewstitleRetrievalPage" />
