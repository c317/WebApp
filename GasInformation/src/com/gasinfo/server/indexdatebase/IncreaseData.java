...
public class IncreaseData {
	...
	public void indexAnddatebase() throws Exception {
		...
		File[] files = new File(PreURL).listFiles();
		for (int  i= 0; i < files.length; i++) {
			PreURL=files[i].getPath();

			this.EnterIndexDateBase("Gzdt", PreURL);
			this.EnterIndexDateBase("Kcgl", PreURL);
			this.EnterIndexDateBase("Zcfg", PreURL);
			this.EnterIndexDateBase("Yzrd", PreURL);
			this.EnterIndexDateBase("Gjhz", PreURL);
			this.EnterIndexDateBase("Kjjz", PreURL);
			// this.EnterIndexDateBase("Tpxw");
			this.EnterIndexDateBase("Lddt", PreURL);
			this.EnterIndexDateBase("Tjsj", PreURL);
			this.EnterIndexDateBase("Ktkf", PreURL);
			this.EnterIndexDateBase("Yjxx", PreURL);
		}
	}

	// 各个模块建索引和入库
	public void EnterIndexDateBase(String MK,String PreURL) throws Exception {
		...
		for (int i = 1; i <= file.listFiles().length / 3; i++) {
			String contentpath = PreURL + File.separator + i + "content.txt";
			String messagepath = PreURL + File.separator + i + "message.txt";
			String htmlpath = PreURL + File.separator + i + "html.html";
			try {
			...
			if (checker.check(news)&& this.NotContain(news.getTitle(), listBlackWords)) {
				if (this.Contain(news.getTitle(), listAllWords)) {
					news.setOutput("1");
				} else {
					news.setOutput("0");
				}
...
//			if (checker.check(news)
//					&& this.NotContain(news.getTitle(), listBlackWords)) {
//				if (this.Contain(news.getTitle(), listAllWords)) {
//					news.setOutput("1");
//				} else {
//					news.setOutput("0");
...
//				jdbcNews.setOutput(Integer.valueOf(news.getOutput()));
	...
	public String GetURL() {
		...
		File file = new File(RootURL_IncreaseData + File.separator
				+ String.valueOf(MaxName + 1));
		file.mkdirs();
